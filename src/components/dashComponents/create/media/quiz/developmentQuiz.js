...
import { updateQuiz, deleteQuiz } from "../graphql/createGraphql";
...
class DevQuiz extends Component {
	constructor(props) {
		super(props);
	...
	componentDidMount = async () => {
		...
		if (id) {
			try {
				const { data } = await API.graphql(graphqlOperation(getQuiz, { id }));
				const questionData = data.getQuiz;
	...
	deleteQuizValues = async () => {
		const { id } = this.props.match.params;
		try {
			await API.graphql(graphqlOperation(deleteQuiz, { input: { id } }));
	...
	timerBackup = async () => {
		...
		try {
			await API.graphql(graphqlOperation(updateQuiz, { input: submitQuiz }));
			this.props.history.push("/home");
		} catch (err) {
			console.log("Error occurred", err);
		}
...
export default withStyles(quizStyles, { withTheme: true })(DevQuiz);
