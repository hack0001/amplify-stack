...
import { updateQuiz } from "../graphql/createGraphql";
...
import { createDuplicateQuiz } from "../../../../../graphql/mutations";
import axios from "axios";

class ProdQuiz extends Component {
	constructor(props) {
		super(props);
		this.state = {
			value: 0,
	...
	handleDuplicate = async e => {
		...
		if (
			...
			const duplicateQuiz = {
				...
				production: false,
				original: false,
				duplicateQuizUserId: overviewValues[0].quizUserId
					? overviewValues[0].quizUserId
					: "",
				duplicateQuizOriginalQuizId: this.state.developmentId,
			};

			try {
				await API.graphql(
					graphqlOperation(createDuplicateQuiz, {
						input: duplicateQuiz,
					}),
				);
...
export default withStyles(quizStyles, { withTheme: true })(ProdQuiz);
