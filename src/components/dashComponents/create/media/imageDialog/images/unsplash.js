...
const UnplashUpload = props => {
	...
	const getMoreUnsplash = async () => {
		...
		const url = `https://api.unsplash.com/search/photos/?page=${updatePage}&per_page=${photosPerPage}&query=${searchTerm}&client_id=${process.env.REACT_APP_UNSPLASH_ACCESS_KEY}`;
	...
	const getUnsplash = async e => {
		...
		if (e.keyCode === 13 && unsplashUrl.length > 1) {
			...
			const url = `https://api.unsplash.com/search/photos/?page=${page}&per_page=${photosPerPage}&query=${searchTerm}&client_id=${process.env.REACT_APP_UNSPLASH_ACCESS_KEY}`;
			try {
				const unsplashResult = await axios.get(url, { crossdomain: true });
				setUnsplashImages(unsplashResult.data.results);
			} catch (err) {
		...
		>
			...
			<DialogActions>
				...
				<Button
					disabled={validImage}
					onClick={e => {
						handleOnChange({ [value]: unsplashUrl, [`${value}-embed`]: false });
						setImageDialog(false);
					}}
					color="primary"
				>
