...
import { getDuplicateSlideShow } from "../../../../../../graphql/queries";
import {
	deleteDuplicateSlideShow,
	updateDuplicateSlideShow,
...
class DuplicateDevSlideShow extends Component {
	constructor(props) {
		super(props);
		...
		if (id) {
			try {
				const { data } = await API.graphql(
					graphqlOperation(getDuplicateSlideShow, { id }),
				);

				const slideData = data.getDuplicateSlideShow;
				...
				this.setState({
					...
					slideShowUserId: slideData.user.userId ? slideData.user.userId : null,
					original: slideData.original,
				});
			} catch (err) {
	...
	deleteSlideShowValues = async () => {
		...
		try {
			await API.graphql(
				graphqlOperation(deleteDuplicateSlideShow, { input: { id } }),
			);
	...
	timerBackup = async () => {
		...
		try {
			await API.graphql(
				graphqlOperation(updateDuplicateSlideShow, { input: submitSlideshow }),
			);
			this.props.history.push("/duplicates");
		} catch (err) {
			console.log("Error occurred", err);
...
DuplicateDevSlideShow.propTypes = {
