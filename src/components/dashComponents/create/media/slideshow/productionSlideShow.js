...
import { updateSlideShow } from "../graphql/createGraphql";
...
import { createDuplicateSlideShow } from "../../../../../graphql/mutations";

import axios from "axios";

class ProdSlideShow extends Component {
	constructor(props) {
		super(props);
	...
	handleSubmit = async e => {
		...
		if (
			...
			const submitSlideShow = {
				...
				tags: overviewValues[0].tags ? overviewValues[0].tags : [" "],
			...
			try {
				...
				await API.graphql(
					graphqlOperation(updateSlideShow, { input: submitDevSlideShow }),
				);
				this.props.history.push("/home");
			} catch (err) {
				console.log("Error occurred", err);
	...
	handleDuplicate = async e => {
		...
		if (
			...
			const duplicateSlideShow = {
				...
				production: false,
				original: false,
				duplicateSlideShowUserId: overviewValues[0].slideShowUserId
					? overviewValues[0].slideShowUserId
					: "",
				duplicateSlideShowOriginalSlideShowId: this.state.developmentId,
			};

			try {
				await API.graphql(
					graphqlOperation(createDuplicateSlideShow, {
						input: duplicateSlideShow,
					}),
				);
...
ProdSlideShow.propTypes = {
	classes: PropTypes.object.isRequired,
};

export default withStyles(slideStyles, { withTheme: true })(ProdSlideShow);
